...
.tiny-credit a:hover{ text-decoration:underline; }
</style>

<div id="widget">
  <div class="header">
    <h2>Continuous Learning City RL Simulator v9</h2>
    <p>
      A continuously running city RL playground: the agent explores chores throughout the simulated day, updates a simple learned value memory,
      resets its daily experience at midnight, and the city clock drives the day/night cycle with weather-independent nighttime city lighting, true window emissive maps, natural energy constraints, and livelier pedestrians.
    </p>
  </div>
  ...
  <div class="toolbar">
    ...
    <button id="learnModeBtn">Pause Learner</button>
    <button id="taskBtn">Start Task</button>
    <button id="autoBtn">Watch Demo Agent</button>
    <button id="timeBtn">Toggle Day / Night</button>
    <button id="cameraBtn">Reset Camera</button>
    <button id="episodeBtn">Reset Episode</button>
  ...
  <div id="stageWrap">
    ...
    <div class="overlay taskCard">
      <strong id="taskTitle">Daily Objective: Learn Chore Routine</strong>
      <span id="taskDesc">Navigate back to a cozy home while avoiding collisions and managing energy.</span>
      <span class="small" id="taskRewardInfo">Reward: chore bonuses minus time, energy, traffic-rule, vehicle-collision, and pedestrian-collision penalties.</span>
    </div>
<canvas id="minimap" width="250" height="210"></canvas>
  ...
  <div id="rl-hidden-state-values" style="display:none;">
    <span id="simClockText">06:00</span>
    <span id="dayText">1</span>
    <span id="epsilonText">0.45</span>
    <span id="bestChoreText">—</span>
    <span id="lastRewardText">0.00</span>
    <span id="returnText">0.0</span>
    <span id="targetText">Home A</span>
    <span id="signalText">Vertical Green</span>
    <span id="batteryText">100%</span>
    <span id="stationText">Available</span>
    <span id="weatherText">Clear</span>
    <span id="violationText">0</span>
    <span id="pedHitText">0</span>
    <span id="collisionText">0</span>
    <span id="packageText">None</span>
    <canvas id="minimap" width="4" height="4"></canvas>
  </div>
...
function resetDailyExperience(incrementDay=true){
  ...
  autoMode = true;
  learningMode = true;
  learnModeBtn.textContent = "Pause Learner";
  ...
  resetCamera();
  statusText.textContent = incrementDay ? "New day started" : "Continuous learner initialized";
...
function resetMainPosition(){
  ...
  autoMode = false;
  learningMode = false;
  learnModeBtn.textContent = "Resume Learner";
  mainAgent.path = [];
  clickMarker.visible = false;
...
function startTask(kind){
  ...
  if(kind === "home"){
    ...
    taskTitle.textContent = "Chore: Get Ice Cream";
    taskDesc.textContent = "The agent may choose this optional chore if it has learned the trip is worthwhile.";
    ...
    taskTitle.textContent = "Chore: Pickup & Dropoff";
    ...
    currentTask.phase = "pickup";
    taskTitle.textContent = "Chore: Multi-Delivery";
    taskDesc.textContent = "The agent tries a longer delivery chain and learns whether the return is worth the cost.";
    ...
    taskTitle.textContent = "Chore: Rest";
  ...
  if(learningMode || autoMode){
    ...
    autoBtn.textContent = "Continuous Learner";
  }
}
...
function setMainTargetFromPointer(event){
  ...
  learnModeBtn.textContent = "Resume Learner";
  mainAgent.path = [];
  autoBtn.textContent = "Watch Demo Agent";
...
autoBtn.addEventListener("click", ()=>{
  ...
  if(autoMode){
    ...
    mainAgent.path = planPath(mainAgent.x, mainAgent.z, currentTarget.x, currentTarget.z);
    drawPolicyPath(mainAgent.path, 0x2563eb, true);
    autoBtn.textContent = learningMode ? "Continuous Learner" : "Back to Manual";
  } else {
    mainAgent.path = [];
    clearGroup(pathGroup);
    autoBtn.textContent = "Watch Demo Agent";
...
learnModeBtn.addEventListener("click", () => {
  ...
  if(learningMode){
    ...
    mainAgent.path = [];
    clearGroup(pathGroup);
    statusText.textContent = "Learner paused";
  }
});
...
function animate(){
  ...
  updateStats();
  drawRewardGraph();
  if (minimapCanvas && minimapCanvas.offsetParent !== null) drawMinimap();
