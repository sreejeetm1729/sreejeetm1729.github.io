...
<div id="rl3d-drone-widget">
  <div class="rl3d-header">
    ...
    <p>
      ...
      hover inside a moving zone, deliver cargo, collect beacons, or fly through gates.
    </p>
  </div>
  ...
  <div class="rl3d-toolbar">
    ...
    <label>
      ...
      <select id="rl3d-task-select">
        <option value="hover">Hover in Reward Zone</option>
        <option value="delivery">Carry Cargo</option>
        <option value="beacons">Collect Beacons</option>
        <option value="gates">Fly Through Gates</option>
      </select>
    </label>
  </div>
  ...
  <div id="rl3d-stage-wrap">
    ...
    <div id="rl3d-task-card">
      <div class="rl3d-task-title" id="rl3d-task-title">Hover in Reward Zone</div>
      <div class="rl3d-task-desc" id="rl3d-task-desc">
        Stay inside the moving green zone.
      </div>
    </div>
...
(function () {
  ...
  scene.fog = new THREE.Fog(0xbfe4ff, 13, 38);

  const camera = new THREE.PerspectiveCamera(50, 1, 0.1, 100);
  camera.position.set(0, 8.8, 12.2);
  camera.lookAt(0, 0, 0);
  ...
  const ambient = new THREE.HemisphereLight(0xffffff, 0x64748b, 1.75);
  scene.add(ambient);

  const sun = new THREE.DirectionalLight(0xffffff, 2.55);
  sun.position.set(-4.5, 9.0, 5.5);
  sun.castShadow = true;
  ...
  function loadEnvironment(name) {
    ...
    if (name === "warehouse") {
      ...
      scene.fog = new THREE.Fog(0xcbd5e1, 13, 35);
      createWarehouse();
    }
  ...
  function createSkyPark() {
    ...
    const grid = new THREE.GridHelper(18, 18, 0x15803d, 0x22c55e);
    grid.material.transparent = true;
    grid.material.opacity = 0.22;
    grid.position.y = 0.012;
    envGroup.add(grid);

    addCloud(-5.5, 4.1, -3.2, 1.0);
    addCloud(5.2, 4.6, -2.7, 0.85);
    addCloud(1.8, 5.1, 3.3, 0.75);

    addPond(-4.9, 2.6, 1.2, 0.72);
    ...
    treePositions.forEach((p, idx) => {
      addTree(p[0], p[1], 0.8 + 0.18 * (idx % 3));
    });

    addBalloon(-6.0, -0.9, 2.5, 0xff5555);
    addBalloon(6.3, 1.0, 2.8, 0x60a5fa);
    addBalloon(0.0, 4.0, 2.9, 0xfacc15);
  }
  ...
  function createWarehouse() {
    ...
    const grid = new THREE.GridHelper(18, 18, 0x334155, 0x64748b);
    grid.material.transparent = true;
    ...
    const cratePositions = [
      [-5.8, -3.5, 0.7], [-4.7, -3.3, 0.9], [-3.2, 3.7, 0.8],
      [3.9, -3.6, 0.75], [5.9, 3.2, 0.9], [1.3, 4.2, 0.65],
      [-6.2, 2.2, 0.7], [6.2, -1.7, 0.75]
    ];

    cratePositions.forEach((p, idx) => {
      addCrate(p[0], p[1], p[2], idx);
    });

    addShelf(-6.7, 0.0, 2.6);
    addShelf(6.7, 0.0, 2.6);
    addSafetyCone(-1.9, -3.8);
    addSafetyCone(-1.2, -3.8);
    addSafetyCone(-0.5, -3.8);

    addObstacleCylinder(0.0, 0.0, 0.45, 1.3, 0xf97316, "pillar");
    addObstacleCylinder(2.4, 1.4, 0.36, 1.0, 0xf97316, "pillar");
    addObstacleCylinder(-2.5, -1.2, 0.36, 1.0, 0xf97316, "pillar");
  }
  ...
  function createNeonCity() {
    ...
    const grid = new THREE.GridHelper(18, 18, 0x38bdf8, 0x7c3aed);
    grid.material.transparent = true;
    grid.material.opacity = 0.42;
    grid.position.y = 0.014;
    envGroup.add(grid);

    addNeonRoad(0, 0, 16.5, 0.12, 0x38bdf8);
    addNeonRoad(0, 0, 0.12, 10.5, 0xa855f7);

    const towers = [
      [-6.4, -4.0, 0.9, 2.1, 0x38bdf8],
      [-5.2, 3.7, 0.75, 1.7, 0xa855f7],
      [-3.5, -4.3, 0.65, 1.4, 0xf97316],
      [4.2, -4.0, 0.7, 1.8, 0x22c55e],
      [6.3, 3.7, 0.85, 2.2, 0x38bdf8],
      [3.1, 4.3, 0.6, 1.5, 0xec4899]
    ];

    towers.forEach((p) => addNeonTower(p[0], p[1], p[2], p[3], p[4]));

    addNeonArch(-4.7, 0.0, 0x38bdf8);
    addNeonArch(4.7, 0.0, 0xa855f7);

    addObstacleCylinder(-2.2, 2.1, 0.38, 1.2, 0x38bdf8, "neon-pillar");
    addObstacleCylinder(2.5, -1.9, 0.38, 1.2, 0xa855f7, "neon-pillar");
  }

  function createArenaBorder() {
    const mat = new THREE.MeshStandardMaterial({
      color: currentEnvironment === "neon" ? 0x38bdf8 : 0x1e293b,
      emissive: currentEnvironment === "neon" ? 0x0ea5e9 : 0x000000,
      emissiveIntensity: currentEnvironment === "neon" ? 0.45 : 0.0,
      roughness: 0.45,
      metalness: 0.08
    });
  ...
  function initTask(task) {
    ...
    if (task === "hover") {
      taskTitle.textContent = "Hover in Reward Zone";
      taskDesc.textContent = "Stay inside the moving green cylinder. The zone moves slowly, so the drone must keep tracking it.";
    ...
    if (task === "delivery") {
      taskTitle.textContent = "Carry Cargo";
      taskDesc.textContent = "Pick up the blue cargo box, carry it under the drone, and drop it inside the orange delivery zone.";

      taskState.pickup = { x: -5.1, z: -2.7 };
      taskState.dropoff = { x: 5.3, z: 2.6 };
      taskState.hasCargo = false;

      taskState.pickupPad = createPad(taskState.pickup.x, taskState.pickup.z, 0x2563eb, 0.78);
      taskState.dropoffPad = createPad(taskState.dropoff.x, taskState.dropoff.z, 0xf97316, 0.95);

      taskState.cargo = createCargoBox();
      taskState.cargo.position.set(taskState.pickup.x, 0.32, taskState.pickup.z);
    ...
    if (task === "beacons") {
      taskTitle.textContent = "Collect Beacons";
      taskDesc.textContent = "Collect all glowing beacons. Each beacon gives a reward burst when the drone reaches it.";

      taskState.beacons = [];
      taskState.currentBeacon = 0;

      const positions = [
        [-5.5, -3.2], [-1.8, 3.7], [3.0, -3.5],
        [5.7, 2.7], [-5.8, 2.2], [1.1, 0.3]
      ];

      positions.forEach((p, idx) => {
        const beacon = createBeacon(idx);
        beacon.position.set(p[0], 1.05, p[1]);
        taskState.beacons.push({
          x: p[0],
          z: p[1],
          mesh: beacon,
          collected: false
        });
        taskGroup.add(beacon);
      });
    }

    if (task === "gates") {
      taskTitle.textContent = "Fly Through Gates";
      taskDesc.textContent = "Fly through the glowing gates in order. The active gate is bright green.";

      taskState.gates = [];
      taskState.gateIndex = 0;
      ...
      gates.forEach((g, idx) => {
        const gate = createGate(idx);
        gate.position.set(g[0], 1.25, g[1]);
        gate.rotation.y = g[2];

        taskState.gates.push({
          x: g[0],
          z: g[1],
          mesh: gate,
          passed: false
  ...
  function step(dt) {
    ...
    const action = policyMode ? getPolicyAction(wind, taskGoal) : getHumanAction();
    ...
    const taskRewardInfo = updateTask(dt);

    instantReward = taskRewardInfo.instantReward;
    totalReward += taskRewardInfo.continuousReward * dt * 10 + taskRewardInfo.eventReward;

    updateDroneVisual(action, taskRewardInfo.closeness);
    updateControlMarker();
    updateTrail();
  ...
  function updateTask(dt) {
    ...
    if (currentTask === "hover") {
      ...
      const tz = 2.15 * Math.sin(0.25 * elapsed + 1.2);
      ...
      if (inside) {
        taskState.successTime += dt;
      }

      progressLabel = taskState.successTime.toFixed(1) + "s";
    }

    if (currentTask === "delivery") {
      ...
      if (!taskState.hasCargo) {
        taskState.cargo.position.set(taskState.pickup.x, 0.32, taskState.pickup.z);
        taskState.cargo.rotation.y += 0.012;
        ...
        taskState.cargo.position.set(droneState.x, DRONE_ALTITUDE - 0.58, droneState.z);
        taskState.cargo.rotation.y += 0.045;
      ...
      pulseObject(taskState.pickupPad, 1.0 + 0.04 * Math.sin(elapsed * 3));
      pulseObject(taskState.dropoffPad, 1.0 + 0.04 * Math.sin(elapsed * 3 + 1.3));

      progressLabel = taskState.deliveries + " delivered";
    }

    if (currentTask === "beacons") {
      let remaining = taskState.beacons.filter(b => !b.collected);

      if (remaining.length === 0) {
        taskState.beacons.forEach((b, idx) => {
          const p = randomArenaPoint();
          b.x = p.x;
          b.z = p.z;
          b.collected = false;
          b.mesh.visible = true;
          b.mesh.position.set(b.x, 1.05, b.z);
        });

        taskState.collected = 0;
        remaining = taskState.beacons;
        eventReward += 6.0;
      }
      ...
      remaining.forEach((b) => {
        const dist = len2(droneState.x - b.x, droneState.z - b.z);
        if (dist < bestDist) {
          bestDist = dist;
          nearest = b;
        }
      ...
      closeness = clamp(1 - bestDist / 5.5, 0, 1);
      continuousReward = 0.22 * closeness;

      taskState.beacons.forEach((b, idx) => {
        if (!b.collected) {
          b.mesh.rotation.y += 0.035;
          b.mesh.position.y = 1.05 + 0.14 * Math.sin(elapsed * 2.8 + idx);

          const d = len2(droneState.x - b.x, droneState.z - b.z);
          if (d < 0.62) {
            b.collected = true;
            b.mesh.visible = false;
            taskState.collected += 1;
            eventReward += 3.0;
          }
        }
      });

      progressLabel = taskState.collected + " / " + taskState.beacons.length;
    }

    if (currentTask === "gates") {
      const active = taskState.gates[taskState.gateIndex];

      if (active) {
        const dist = len2(droneState.x - active.x, droneState.z - active.z);
        closeness = clamp(1 - dist / 5.5, 0, 1);
        continuousReward = 0.20 * closeness;

        if (dist < 0.78) {
          active.passed = true;
          taskState.gateIndex += 1;
          eventReward += 3.5;
          updateGateColors();
        }
      }

      if (taskState.gateIndex >= taskState.gates.length) {
        eventReward += 7.5;

        taskState.gates.forEach((g, idx) => {
          const p = randomArenaPoint();
          g.x = p.x;
          g.z = p.z;
          g.passed = false;
          g.mesh.position.set(g.x, 1.25, g.z);
          g.mesh.rotation.y = -0.8 + idx * 0.55;
        });
      ...
      taskState.gates.forEach((g, idx) => {
        g.mesh.rotation.z = 0.08 * Math.sin(elapsed * 2.0 + idx);
      });

      progressLabel = taskState.gateIndex + " / " + taskState.gates.length;
    }
  ...
  function getTaskGoal() {
    if (currentTask === "hover" && taskState.target) {
      return {
        x: taskState.target.group.position.x,
        z: taskState.target.group.position.z
      };
    }

    if (currentTask === "delivery") {
      return taskState.hasCargo ? taskState.dropoff : taskState.pickup;
    }

    if (currentTask === "beacons") {
      const remaining = taskState.beacons.filter(b => !b.collected);
  ...
  function getHumanAction() {
    ...
    return {
      ax: maxAccel * strength * dx / d,
      az: maxAccel * strength * dz / d
    };
  }
  ...
  function getWind() {
    ...
    return {
      x: 0.92 * Math.sin(0.72 * elapsed + 0.55),
      z: 0.70 * Math.cos(0.57 * elapsed + 1.35)
    };
  }

  function handleArenaBounds() {
    if (droneState.x < -ARENA_X) {
      droneState.x = -ARENA_X;
      droneState.vx *= -0.35;
    }
    ...
    if (droneState.z < -ARENA_Z) {
      droneState.z = -ARENA_Z;
      droneState.vz *= -0.35;
    }
  ...
  function updateDroneVisual(action, closeness) {
    const altitude = DRONE_ALTITUDE + 0.07 * Math.sin(elapsed * 5.5);
    ...
    droneGroup.rotation.set(pitch, 0.25 * Math.sin(elapsed * 0.8), roll);

    const insideColor = new THREE.Color(0x22c55e);
    const outsideColor = new THREE.Color(0x2563eb);
    const bodyColor = outsideColor.clone().lerp(
      insideColor,
      clamp((closeness - 0.55) / 0.45, 0, 1)
    );
    ...
    droneGroup.userData.navLights.forEach((light, idx) => {
      const blink = 0.65 + 0.35 * Math.sin(elapsed * 8 + idx);
  ...
  function updateControlMarker() {
    ...
    controlMarker.position.set(controlPoint.x, 0.07, controlPoint.z);
  ...
  function updateTrail() {
    ...
    for (let i = 0; i < p.length; i++) {
      ordered.push(p[(trail.index + i) % p.length]);
    }
  ...
  function updateWindVector(wind) {
    ...
    windVectorGroup.visible = windOn && speed > 0.01;

    const angle = Math.atan2(wind.z, wind.x);
    windVectorGroup.rotation.y = -angle;

    const scale = 0.65 + 0.55 * clamp(speed / 1.2, 0, 1);
    windVectorGroup.scale.set(scale, scale, scale);
  }
  ...
  function updateCompass(wind) {
    ...
    compass.scene.rotation.y = 0.18 * Math.sin(elapsed * 0.7);
  }

  function updateCamera() {
    const desiredX = droneState.x * 0.12;
    const desiredZ = 12.2 + droneState.z * 0.08;

    camera.position.x += (desiredX - camera.position.x) * 0.018;
    camera.position.z += (desiredZ - camera.position.z) * 0.018;
    camera.lookAt(droneState.x * 0.08, 0.3, droneState.z * 0.08);
  }
  ...
  function createDrone() {
    ...
    const lens = new THREE.Mesh(
      new THREE.CylinderGeometry(0.075, 0.075, 0.045, 24),
      glassMat
    );
    ...
    const nose = new THREE.Mesh(
      new THREE.ConeGeometry(0.13, 0.28, 28),
      lightBlueMat
    );
    ...
    rotorPositions.forEach((p, idx) => {
      ...
      const hub = new THREE.Mesh(
        new THREE.CylinderGeometry(0.105, 0.105, 0.11, 32),
        darkMat
      );
      hub.castShadow = true;
      ...
      const blade1 = new THREE.Mesh(
        new THREE.BoxGeometry(0.58, 0.018, 0.075),
        propMat
      );
      ...
      const blade2 = new THREE.Mesh(
        new THREE.BoxGeometry(0.075, 0.018, 0.58),
        propMat
      );
      ...
      rotorBlur.position.y = 0.083;
      propeller.add(rotorBlur);

      rotorAssembly.add(propeller);
      group.userData.rotors.push(propeller);

      const lightColorMat = idx < 2 ? redLightMat : greenLightMat;
    ...
    const skidLeft = createLandingSkid(-0.38, darkMat);
    const skidRight = createLandingSkid(0.38, darkMat);
    group.add(skidLeft);
    group.add(skidRight);
  ...
  function createCargoBox() {
    ...
    const strap2 = new THREE.Mesh(
      new THREE.BoxGeometry(0.08, 0.04, 0.5),
      strapMat
    );
  ...
  function createBeacon(idx) {
    const group = new THREE.Group();

    const colors = [0x38bdf8, 0xa855f7, 0xfacc15, 0x22c55e, 0xec4899, 0xf97316];
  ...
  function createGate(idx) {
    ...
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(0.72, 0.045, 16, 96),
      mat
    );
    ...
    const base = new THREE.Mesh(
      new THREE.CylinderGeometry(0.08, 0.12, 1.0, 18),
      new THREE.MeshStandardMaterial({
        color: 0x334155,
        roughness: 0.5,
        metalness: 0.18
  ...
  function updateGateColors() {
    ...
    taskState.gates.forEach((g, idx) => {
      ...
      if (g.passed) {
        ...
        ring.material.emissive.set(0x334155);
        ring.material.emissiveIntensity = 0.25;
      } else if (idx === taskState.gateIndex) {
        ring.material.color.set(0x22c55e);
        ring.material.emissive.set(0x22c55e);
        ring.material.emissiveIntensity = 1.1;
      } else {
        ring.material.color.set(0x38bdf8);
        ring.material.emissive.set(0x0ea5e9);
  ...
  function createWindVector3D() {
    ...
    const shaft = new THREE.Mesh(
      new THREE.CylinderGeometry(0.035, 0.035, 1.0, 18),
      mat
    );
  ...
  function createCompassRenderer() {
    ...
    const light = new THREE.HemisphereLight(0xffffff, 0x334155, 2.2);
    ...
    const shaft = new THREE.Mesh(
      new THREE.BoxGeometry(0.13, 0.055, 0.9),
      needleMat
    );
    ...
    const head = new THREE.Mesh(
      new THREE.ConeGeometry(0.18, 0.36, 32),
      needleMat
    );
  ...
  function addObstacleCylinder(x, z, r, h, color, kind) {
    const mat = new THREE.MeshStandardMaterial({
      color,
      emissive: kind === "neon-pillar" ? color : 0x000000,
      emissiveIntensity: kind === "neon-pillar" ? 0.5 : 0.0,
      roughness: 0.4,
      metalness: 0.12
    });
  ...
  function addTree(x, z, scale) {
    ...
    const crown = new THREE.Mesh(
      new THREE.ConeGeometry(0.42 * scale, 0.9 * scale, 24),
      new THREE.MeshStandardMaterial({ color: 0x16a34a, roughness: 0.8 })
    );
    crown.position.set(x, 0.98 * scale, z);
  ...
  function addCloud(x, y, z, scale) {
    ...
    for (let i = 0; i < 5; i++) {
      const puff = new THREE.Mesh(
        new THREE.SphereGeometry((0.28 + 0.08 * (i % 2)) * scale, 18, 12),
        mat
      );
      puff.position.set((i - 2) * 0.25 * scale, 0.05 * Math.sin(i), 0);
      group.add(puff);
    }
  ...
  function addCrate(x, z, size, idx) {
    ...
    crate.rotation.y = 0.25 * idx;
    crate.castShadow = true;
    crate.receiveShadow = true;
    envGroup.add(crate);

    obstacles.push({ x, z, r: size * 0.55 });
  }

  function addShelf(x, z, height) {
    const mat = new THREE.MeshStandardMaterial({
      color: 0x475569,
      roughness: 0.48,
      metalness: 0.18
    ...
    for (let i = 0; i < 4; i++) {
      const board = new THREE.Mesh(
        new THREE.BoxGeometry(0.35, 0.08, 4.6),
        mat
      );
      board.position.set(0, 0.45 + i * 0.55, 0);
  ...
  function addNeonTower(x, z, width, height, color) {
    ...
    const glow = new THREE.Mesh(
      new THREE.BoxGeometry(width * 1.02, 0.04, width * 1.02),
      new THREE.MeshBasicMaterial({
        color,
        transparent: true,
        opacity: 0.65
  ...
  function randomArenaPoint() {
    return {
      x: -6.2 + Math.random() * 12.4,
      z: -4.0 + Math.random() * 8.0
    };
  }
  ...
  function pulseObject(obj, scale) {
  ...
  policyBtn.addEventListener("click", function () {
    ...
    policyBtn.textContent = policyMode
      ? "Back to Human Control"
      : "Watch Demo Controller";
  });
