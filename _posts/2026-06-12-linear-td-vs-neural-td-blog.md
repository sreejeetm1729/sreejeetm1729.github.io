...
:=
\bigl\{x\mapsto \phi(x)^{\texttt{T}}\theta:\theta\in\Theta\bigr\}.
$$
...
\delta_\theta(x,r,x')
:=
\phi(x)^{\texttt{T}}\theta-r-\gamma \phi(x')^{\texttt{T}}\theta.
$$
...
A
:=
\mathbb{E}\bigl[\phi(x)(\phi(x)-\gamma\phi(x'))^{\texttt{T}}\bigr],
\qquad
b
:=
...
\left\{
 ...
 b_r\mathbf{1}\{W_r(0)^{\texttt{T}} x>0\} W_r^{\texttt{T}} x
 :
 W\in S_B
\right\}.
$$
